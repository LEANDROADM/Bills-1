...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ Calmae Zé estou fazendo bacana< ⌛',
				success: '✔️ Sucesso ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando só pode ser usado pelo bills! ❌',
			...
			const ownerNumber = ["5567920018881@s.whatsapp.net"] // replace this with your number
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Onde está o url em Zé?')
					...
					break
				case 'ytmp4':
					if (args.length < 1) return reply('Onde está o url, bacana?')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
		...
		if (args.length < 1) return reply ('*PARCEIROS*: \nEaxy: https://youtube.com/c/EAXYFF\nYamii: https://youtube.com/c/TioYamiiroivemsmpaki\nSaad: https://youtube.com/channel/UCtX3fzhC-teLUpaS7Xz8nKA\nKazuma: https://youtube.com/channel/UChRgBlar0IKGAsseuBeNRgA')
				...
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						client.sendMessage(from,`Atencao a todos os membros ativos deste grupo anti-link. ee você enviar um link de grupo, voce sentira o gosto de ser expulso ok<`, text)
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 BILLS BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 BILLS BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 BILSS BOT 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
